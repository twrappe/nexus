...
- [config/contracts/event_class_output.yaml](config/contracts/event_class_output.yaml) — `/ai/event_class` message contract; field types, valid values, confidence bounds (FM-04, FM-05)
...
- [config/thresholds/latency.yaml](config/thresholds/latency.yaml) — host inference <= 20 ms; STM32 nominal <= 50 ms, stressed <= 75 ms; mode transition <= 10 ms (FM-01, FM-08)
- [config/thresholds/rates.yaml](config/thresholds/rates.yaml) — INT8/FP32 agreement >= 98%; critical miss rate = 0; false critical rate <= 1%; contract compliance = 100%; autonomous CRITICAL recovery = 0 (FM-02, FM-03, FM-06, FM-07, FM-09)
...
| File | Failure Mode |
|------|-------------|
| `test_timing_nominal.py` | FM-01 — Inference latency budget |
| `test_quantization_divergence.py` | FM-02 — INT8 vs FP32 classification |
| `test_false_critical.py` | FM-03 — NOMINAL input → CRITICAL_EVENT output |
| `test_contract_confidence.py` | FM-04 — Confidence outside contract range |
| `test_fault_dropped_input.py` | FM-05 — Dropped input → stale output propagated |
| `test_critical_event_miss.py` | FM-06 — CRITICAL_EVENT input → NOMINAL output |
| `test_fault_ood_input.py` | FM-07 — OOD input → silent confident NOMINAL |
| `test_edge_timing_hil.py` | FM-08 — micro-ROS latency exceeds budget |
| `test_critical_terminal.py` | FM-09 — Autonomous recovery from CRITICAL state |
...
- `docs/fmea/fmea_table.md` — full FMEA table with RPN rationale (referenced by README)
